define(["./EventHandler"], function (EventHandler) {
    ...
    function MetaDataRenderer(args) {
        ...
        var renderTask = function (task) {
            var s = new Section({domNode: domNode});
            s.setName(task.getType());
            ["GlobalId", "Name", "TaskId"].forEach(function (k) {
                var fn = task["get" + k];
                if (fn) {
                    r = s.addRow();
                    r.setName(k);
                    r.setValue(fn.apply(task));
                }
            });
            task.getHasAssignments(function (assignment) {
                assignment.getTimeForTask(function (timeForTask) {
                    timeForTask.getScheduleStart(function (scheduleStart) {
                        if (scheduleStart.getType() == "IfcDateAndTime") {
                            scheduleStart.getDateComponent(function (dateComponent) {
                                r = s.addRow();
                                r.setName("ScheduleStart");
                                r.setValue(renderDateAndTime(dateComponent));
                            });
                        }
                    });
                    timeForTask.getScheduleFinish(function (scheduleFinish) {
                        if (scheduleFinish.getType() == "IfcDateAndTime") {
                            scheduleFinish.getDateComponent(function (dateComponent) {
                                r = s.addRow();
                                r.setName("ScheduleFinish");
                                r.setValue(renderDateAndTime(dateComponent));
                            });
                        }
                    });
        ...
        var renderDateAndTime = function (dateComponent) {
            return dateComponent.getYearComponent() + "-" + dateComponent.getMonthComponent() + "-" + dateComponent.getDayComponent();
        };
        ...
        this.setSelected = function (oid) {
            ...
            o.getHasAssignments(function (hasAssignments) {
                console.log("getHasAssignments!");
                if (hasAssignments.getType() == "IfcRelAssignsToProcess") {
                    console.log("IfcRelAssignsToProcess!");
                    hasAssignments.getRelatingProcess(function (process) {
